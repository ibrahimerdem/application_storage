...
def model_base(hparams):
    ...
    if hparams["style"]=="gru":
        if hparams["bidirect"]:
            x = layers.Bidirectional(layers.GRU(units=hparams["RNN_UNITS"],
                                                return_sequences=True))(concat_inputs)
        else:
            x = layers.GRU(units=hparams["RNN_UNITS"],
                           return_sequences=True)(concat_inputs)
    else:
        if hparams["bidirect"]:
            x = layers.Bidirectional(layers.LSTM(units=hparams["RNN_UNITS"],
                                                 return_sequences=True))(concat_inputs)
        else:
            x = layers.LSTM(units=hparams["RNN_UNITS"],
                            return_sequences=True)(concat_inputs)
    x = layers.Dropout(rate=hparams["DROP_RATE"])(x)
    ...
    model.compile(loss=loss_function,
                  optimizer=keras.optimizers.Adam(learning_rate=hparams["L_RATE"]),
